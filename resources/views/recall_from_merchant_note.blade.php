...
<html>
<head>
    <title>Recall from Vendor Note</title>
...
<body>
    ...
    <div class="table-responsive" style="margin-top:25px;">
        <table class="table">
            ...
            <tbody>
            @foreach($dataset['recall__terminal'] as $item)
                <tr>
                    <td>{{ $item['tid'] }}</td>
                    <td>{{ $item['merchant'] }} ({{ $item['mid'] }})<br/>{{ $item['city']}}</td>
                    <td>{{ $item['product_name'] }}<br/>SN: {{ $item['serial_no'] }}<br/>Connection: {{ $item['connection_type'] }}</td>
